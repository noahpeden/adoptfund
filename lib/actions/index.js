...
export const newBasics = (newBasics) => {
  return {
    type: 'NEW_BASICS',
    newBasics,
  }
...
export const createBasics = (title, location, expiration) => {
  return (dispatch) => {
    return fetch('https://adoptfund-api.herokuapp.com/api/v1/family', {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({ title, location, expiration } )
    })
    .then(data => data.json())
    .then(data => {
      dispatch(newBasics(data))
    })
    .catch(err => console.log(err))
  }
}
